...
func (g *Generator) WriteHeader() {
	g.buf.WriteString("// Code generated by fastjson for scanning JSON strings. DO NOT EDIT.\n")
...
func makeUnmarshalDefine(
	...
	if tmpl, ok := readFnMap[tn]; ok {
		...
		if typ.arr {
			fn = "    *o, err = (*" + tn + ")(nil).UnmarshalFastJSONSlice(r)" + "\n"
		} else {
			fn = "    err = o.UnmarshalFastJSON(r)" + "\n"
		}
	}

	return "func (o *" + name + ")UnmarshalFastJSON(r *reader.Reader) (err error) {\n" +
		fn +
		"    return\n" +
		"}\n\n" +
		"func (o *" + name + ")sequenceFastJSON(r *reader.Reader, idx int) (res []" + name + ", err error) {\n" +
		"    var e " + name + "\n" +
		...
		"    return\n" +
		"}\n\n" +
		"func (o *" + name + ") UnmarshalFastJSONSlice(r *reader.Reader) (res []" + name + ", err error) {\n" +
...
func makeUnmarshalStruct(
	...
	for i, e := range fields {
		...
		if tmpl, ok := readFnMap[tn]; ok {
			...
			if types[i].arr {
				fns[i] = fmt.Sprintf("                    case \"%s\":\n", aliases[i]) +
					"                        o." + e + ", err = (*" + tn + ")(nil).UnmarshalFastJSONSlice(r)"
			} else if types[i].ptr {
				fns[i] = fmt.Sprintf("                    case \"%s\":\n", aliases[i]) +
					"                        o." + e + " = &" + tn + "{}\n" +
					"                        err = o." + e + ".UnmarshalFastJSON(r)"
			} else {
				fns[i] = fmt.Sprintf("                    case \"%s\":\n", aliases[i]) +
					"                        err = o." + e + ".UnmarshalFastJSON(r)"
			}
